...
**Keep-Alive** — 일정 주기(10s)로 빈 패킷을 보내 세션 생존을 알리고, NAT 매핑이 만료되지 않게 유지합니다.

**Rekey** — 일정 주기(10s)로 세션 키를 새로 생성·교체합니다. 하나의 키로 암호화하는 데이터 양·시간을 제한해 키 노출 시 피해를 줄입니다(forward secrecy 방향의 완화책).
...
Client                                  Server
  ...
  |                                        |
  |  ── KEEP_ALIVE (10s 주기) ───────────► |   (세션/NAT 유지)
  |                                        |
  | REKEY (10s): 새 AES 키 생성             |
...
- Client는 `REKEY_INTERVAL(10s)`마다 새 AES 키를 만들어 서버 공개키로 암호화해 전송하고, `KEEP_ALIVE_INTERVAL(10s)`마다 keep-alive를 보냅니다.
...
6) DATA (터널 통신)    ping 8.8.8.8 (터널 경유)
        ↓
7) KEEP_ALIVE         10초마다 [CLIENT] KEEP_ALIVE / [SERVER] KEEP_ALIVE
        ↓
8) REKEY              10초마다 [CLIENT] Rekey Start → Rekey Sent
        ↓
...
**4. Keep-Alive** — 10초 간격으로 양쪽에 KEEP_ALIVE 로그가 반복됩니다.
...
**5. Rekey** — 10초마다 Client가 새 AES 키를 만들어 재전송합니다.
